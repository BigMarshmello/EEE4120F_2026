...
function run_analysis()
    ...
    for k = 1:numel(images)
        ...
        % d) Correctness
        difference = double(result1) - double(result2);
        mean_difference = mean(abs(difference(:)));

       
        fprintf(file,"Size %d x %d\n", x, y);

        fprintf(file,"Average Execution Time for Manual:  %.6f s\n", average_manual);
        fprintf(file,"Std Dev Manual: %.6f s\n", std_manual);

        fprintf(file,"Average Execution Time for Inbuilt: %.6f s\n", average_inbuilt);
        fprintf(file,"Std Dev Inbuilt: %.6f s\n", std_inbuilt);

        fprintf(file,"Speedup (manual/inbuilt): %.6f\n", speedup);

        fprintf(file,"Average difference (manual - builtin): %.6f\n", mean_difference);
