...
module StarCore1_tb;
    ...
    initial begin
        ...
        // -----------------------------------------------------------------------
         $display("Checking R3 after SUB R1-R0 (expect 0x0002-0x0001 = 0x0001):");
         check16(uut.DU.reg_file.reg_array[3], 16'h0001, test_id);
         test_id = test_id + 1;

        $display("Checking R4 after AND R0&R1 (expect 0x0001&0x0002 = 0x0000):");
         check16(uut.DU.reg_file.reg_array[4], 16'h0000, test_id);
         test_id = test_id + 1;   

         $display("Checking R5 after OR R0|R1 (expect 0x0001|0x0002 = 0x0003):");
         check16(uut.DU.reg_file.reg_array[5], 16'h0003, test_id);
         test_id = test_id + 1;

         $display("Checking R6 after SLT R0<R1? (expect 0x0001<0x0002? = 0x0001):");
